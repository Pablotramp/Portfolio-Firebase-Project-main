...
export function LoginForm () {
  ...
  const { email, password, handleChange } = useForm({ email: 'aqui@tuEMail', password: '' })
  ...
  return (
    <div className='flex flex-col px-8 pt-6 pb-8 mb-4 bg-white border rounded shadow-md'>
      <div className='mb-4'>
      ...
      <div className='flex items-center justify-start gap-2'>
        <button onClick={handleSubmit} className='px-4 py-2 font-bold text-white rounded bg-slate-500 hover:bg-slate-700' type='button'>
          Acceso
        </button>
      </div>
    </div>
